...
#' 
#' This function gathers the references on the EOL pages (not the references on
#' the provider pages).
#' 
#' 
#' @param MyEOLs A vector of filenames or a list of XMLs for downloaded EOL
#' pages
#' @param output Detail will return a data frame with eolID and reference;
#' counts will return a dataframe with eol taxon name, eol ID, and number of
#' references.
...
#' \code{\link{GetIUCNStat}} \code{\link{DataObjectOverview}}
#' @examples
#' 
#' data(MyEOLs)
