...
#' will be the number of taxa in MyEOLs.  Usually, ITIS, GBIF, and NCBI are top
#' contributors (but this varies depending on scale). \code{BestProvider} will
#' choose the top provider from this list (even if there are ties).
#' \code{DownloadHierarchy} will download the XML information either to a file
...
DownloadHierarchy <- function(
  ...
  results <- GatherProviderDataFrame(
    MyEOLs,
  ...
  hierpages <- vector("list", length = n_pages)
  for (i in sequence(n_pages)) {
    if (!is.na(pages[i])) {
      pageNum<-pages[i]
      web <- paste0("https://eol.org/api/hierarchy_entries/1.0/", pageNum, ".xml")
      if(to.file) {
        write(RCurl::getURL(web), file=paste("hier", pages[i], ".xml", sep=""))
        if(verbose)
          print(paste0("Downloaded ", "hier", pages[i], ".xml"))
      } else {
        hierpages[[i]] <- RCurl::getURL(web)
        names(hierpages)[[i]] <- paste0("hier", pages[i])
        if(verbose)
          print(paste0("hier", pages[i], " saved as R object"))
      }
      # Sys.sleep(1)
    }
  }
