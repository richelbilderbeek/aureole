...
#' objects. For example, it will tell you that there are 3 gifs, 12 jpgs, and
#' 34 text objects.
#' @aliases GatherDataObjectInformation
...
#' \code{CombineDataObjectInformation} will combine the data
#' gathered singly into a huge matrix.  Printing the results from either of these functions may hang up R or cause memory issues, so it isnt recommended (though you can pull of pieces of it).  \code{DataObjectOverview} returns a data frame where each row is a single taxon and columns are a type of data object.
...
#' pages
#' @param verbose An optional print statement while combining
#' @return \code{GatherDataObjectInformation} will return information from one
#' single EOL file all of the data objects and their associated accession info.
#' \code{CombineDataObjectInformation} will combine the data gathered singly
#' into a huge matrix.  Printing the results from either of these functions may
#' hang up R or cause memory issues, so it isnt recommended (though you can
#' pull of pieces of it).  \code{DataObjectOverview} returns a data frame where
#' each row is a single taxon and columns are a type of data object.
