...
#'
#' These functions match hierarchy concept IDs with EOL page data and/or
#' taxonomic tree tips.
#'
#'
#' @aliases MatchHierPageToEOLdata MatchDataToTreeTips
#' @param MyHiers A vector of filenames for downloaded hierarchy pages or a
#' list of XML data stored as an R object
#' @param EOLdata Any output dataframe from Reol functions
#' @return \code{MatchHierPageToEOLdata} returns a data frame where each row is
#' a taxonomic unit. This is especially helpful if you wish to plot EOL data on
#' a taxonomic tree (see example), because it matches the hierarchy taxon names
#' with the EOL data (since taxon names will not always match).
#' \code{MatchDataToTreeTips} will match the tip labels on a tree with those
#' from a data frame.  It orders the data in the same way as is in the trees,
#' so that you can plot data on a tree without distortion (either in number of
#' data points, or in taxonomic name).
#' @examples
#'
